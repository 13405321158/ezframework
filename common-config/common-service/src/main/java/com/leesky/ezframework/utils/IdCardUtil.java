...
public class IdCardUtil {
    ...
    public static Boolean isValidatedAllIdcard(String idcard) {
        Assert.isTrue(StringUtils.isNotBlank(idcard), "身份证号码必须是18位 或 15位");
    ...
    public static boolean validate18Idcard(String idcard) {
        ...
        // 非18位为假
        Assert.isTrue(idcard.length() == 18, "身份证号码有效格式是15位或18位");

        // 获取前17位
        String idcard17 = idcard.substring(0, 17);
        Assert.isTrue(isDigital(idcard17), "身份证号码前17位必须是数字");

        String provinceid = idcard.substring(0, 2);
        // 校验省份
        Assert.isTrue(checkProvinceid(provinceid), "省份代码检验失败：" + provinceid);

        // 校验出生日期
        String birthday = idcard.substring(6, 14);

        Date ss = DateUtils.parseDate(birthday, "yyyyMMdd");

        String dd = DateFormatUtils.format(ss, "yyyyMMdd");

        Assert.isTrue(StringUtils.equals(birthday, dd), "出生日期格式不正确：" + birthday);
