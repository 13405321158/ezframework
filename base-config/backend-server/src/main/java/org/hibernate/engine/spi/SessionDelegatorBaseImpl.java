 ...
 *
 * Wraps and delegates all methods to a {@link SessionImplementor} and a
 * {@link Session}. This is useful for custom implementations of this API so
 * that only some methods need to be overridden (Used by Hibernate Search).
 *
 * @author <a href="mailto:sanne@hibernate.org">Sanne Grinovero</a> (C) 2012 Red
 *         Hat Inc.
 */
...
public class SessionDelegatorBaseImpl implements SessionImplementor {
	...
	/**
	 * @deprecated (since 5.3) SessionDelegatorBaseImpl should take just one
	 *             argument, the SessionImplementor. Use the
	 *             {@link #SessionDelegatorBaseImpl(SessionImplementor)} form
	 *             instead
	 */
	@Deprecated
	...
	/**
	 * Returns the underlying delegate. Be careful that it has a different behavior
	 * from the {@link #getDelegate()} method coming from the EntityManager
	 * interface which returns the current session.
	 *
...
//		return delegate.createNativeQuery( sqlString, resultClass );
	...
	/**
	 * This is an implementation of EntityManager#getDelegate(). It returns the
	 * current session and not the delegate session as it is what we want. The name
	 * of the method is misleading here but, as it is part of JPA, we cannot do
	 * anything about it.
	 * <p>
